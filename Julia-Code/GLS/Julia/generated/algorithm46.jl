...
function algorithm46(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
    ...
    # X2: ml0, full, L21: ml1, lower_triangular, tmp57: ml3, full
    ml4 = Array{Float64}(undef, 250, 500)
    blascopy!(250*500, ml0, 1, ml4, 1)
    # tmp33 = (L21^-1 X2)
    trsm!('L', 'L', 'N', 'N', 1.0, ml1, ml0)

    # X2: ml4, full, L21: ml1, lower_triangular, tmp57: ml3, full, tmp33: ml0, full
    # tmp33 = (L21^-1 X2)
    trsm!('L', 'L', 'N', 'N', 1.0, ml1, ml4)

    # tmp57: ml3, full, tmp33: ml4, full
    ml5 = Array{Float64}(undef, 500, 500)
    # tmp27 = (tmp33^T tmp33)
    syrk!('L', 'T', 1.0, ml4, 0.0, ml5)

    # tmp57: ml3, full, tmp27: ml5, symmetric_lower_triangular
    # (L35 L35^T) = tmp27
    LinearAlgebra.LAPACK.potrf!('L', ml5)

    # tmp57: ml3, full, L35: ml5, lower_triangular
    # tmp59 = (L35^-1 tmp57)
    trsv!('L', 'N', 'N', ml5, ml3)

    # L35: ml5, lower_triangular, tmp59: ml3, full
    # tmp26 = (L35^-T tmp59)
    trsv!('L', 'T', 'N', ml5, ml3)
