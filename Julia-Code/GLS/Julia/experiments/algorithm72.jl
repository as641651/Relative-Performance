...
function algorithm72(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
    ...
    # tmp48: ml5, full, tmp39: ml6, full
    ml7 = Array{Float64}(undef, 500, 500)
    # tmp27 = (tmp48 tmp48^T)
    syrk!('L', 'N', 1.0, ml5, 0.0, ml7)

    # tmp39: ml6, full, tmp27: ml7, symmetric_lower_triangular
    # (L35 L35^T) = tmp27
    LinearAlgebra.LAPACK.potrf!('L', ml7)

    # tmp39: ml6, full, L35: ml7, lower_triangular
    # tmp41 = (L35^-1 tmp39)
    trsv!('L', 'N', 'N', ml7, ml6)

    # L35: ml7, lower_triangular, tmp41: ml6, full
    # tmp26 = (L35^-T tmp41)
    trsv!('L', 'T', 'N', ml7, ml6)
